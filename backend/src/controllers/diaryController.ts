...
export async function getEntry(req: Request, res: Response) {
  try {
    const user = await authService.getUserFromRequest(req)
    if (!user) return res.status(401).json({ error: 'unauthenticated' })
    const entry = await diaryService.get(req.params.id, user.id)
...
export async function updateEntry(req: Request, res: Response) {
  try {
    const user = await authService.getUserFromRequest(req)
    if (!user) return res.status(401).json({ error: 'unauthenticated' })
    const entry = await diaryService.update(req.params.id, user.id, req.body)
    res.json({ entry })
  } catch (err: any) {
    res.status(400).json({ error: err.message })
  }
...
export async function deleteEntry(req: Request, res: Response) {
  try {
    const user = await authService.getUserFromRequest(req)
    if (!user) return res.status(401).json({ error: 'unauthenticated' })
    await diaryService.remove(req.params.id, user.id)
    res.json({ ok: true })
  } catch (err: any) {
    res.status(500).json({ error: err.message })
  }
